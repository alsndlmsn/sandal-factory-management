alter table public.sales add column if not exists cash_register_id uuid references public.cash_registers(id);
alter table public.sales add column if not exists bank_id uuid references public.banks(id);
...
declare s public.sales; item jsonb; prod public.products; sub numeric:=0; disc numeric:=0; vat numeric:=0; total numeric:=0; paid numeric:=0; qty numeric; price numeric; line numeric; pay jsonb;
begin
 ...
 if p_items is null or jsonb_array_length(p_items)=0 then raise exception 'أضف منتجًا واحدًا على الأقل'; end if;
 ...
 values(coalesce(nullif(p_invoice_type,''),'sale'),p_customer_id,coalesce(nullif(btrim(p_customer_name),''),'عميل عادي'),p_warehouse_id,coalesce(nullif(p_discount_type,''),'amount'),greatest(p_discount_value,0),greatest(p_crafting_fee,0),coalesce(p_vat_enabled,false),greatest(p_vat_rate,0),'cash',p_notes,auth.uid()) returning * into s;
 for item in select * from jsonb_array_elements(p_items) loop
   ...
   qty:=greatest((item->>'quantity')::numeric,0); if qty<=0 then raise exception 'الكمية يجب أن تكون أكبر من صفر'; end if;
 ...
 if p_payments is not null then for pay in select * from jsonb_array_elements(p_payments) loop paid:=paid+greatest(coalesce(nullif(pay->>'amount','')::numeric,0),0); insert into public.sale_payments(sale_id,amount,payment_method,cash_register_id,bank_id,reference,created_by) values(s.id,greatest(coalesce(nullif(pay->>'amount','')::numeric,0),0),coalesce(nullif(pay->>'payment_method',''),'cash'),nullif(pay->>'cash_register_id','')::uuid,nullif(pay->>'bank_id',''),nullif(pay->>'reference',''),auth.uid()); end loop; end if;
 if paid>total then raise exception 'إجمالي المدفوع أكبر من قيمة الفاتورة'; end if;
 update public.sales set subtotal=sub,discount_amount=disc,vat_amount=vat,total=total,paid_amount=paid,payment_method=case when paid=0 then 'unpaid' when paid<total then 'partial' else coalesce((select payment_method from public.sale_payments where sale_id=s.id order by created_at limit 1),'cash') end where id=s.id returning * into s;
 return s;
end; $$;
create or replace function public.approve_sale(p_sale_id uuid) returns public.sales
language plpgsql security definer set search_path=public as $$
...
begin
 ...
 for pay in select * from public.sale_payments where sale_id=s.id loop
   if pay.amount>0 and pay.payment_method in('cash','partial_cash') then insert into public.cash_transactions(cash_register_id,direction,transaction_type,amount,related_type,related_id,reference,created_by) values(coalesce(pay.cash_register_id,s.cash_register_id),'in','sale',pay.amount,'sale',s.id,pay.reference,auth.uid()); end if;
   if pay.amount>0 and pay.payment_method in('bank','partial_bank') then insert into public.bank_transactions(bank_id,direction,transaction_type,amount,related_type,related_id,reference,created_by) values(coalesce(pay.bank_id,s.bank_id),'in','sale',pay.amount,'sale',s.id,pay.reference,auth.uid()); end if;
 end loop;
