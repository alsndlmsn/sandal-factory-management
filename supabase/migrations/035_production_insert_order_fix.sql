-- تصحيح ترتيب قيم INSERT في دالة الإنتاج بعد إضافة order_type.
...
begin
  ...
  values(p_product_id,current_date,p_planned_quantity,kind,p_source_warehouse_id,p_raw_material_id,coalesce(p_raw_quantity,0),p_destination_warehouse_id,'draft'::document_status,p_notes,auth.uid()) returning * into po;
  if p_raw_material_id is not null then
