...
declare s public.sales; due numeric; method text;
begin
 ...
 update public.sales set paid_amount=coalesce(paid_amount,0)+p_amount,remaining_amount=greatest(total-(coalesce(paid_amount,0)+p_amount),0),payment_method=case when greatest(total-(coalesce(paid_amount,0)+p_amount),0)=0 then 'cash' else 'partial' end where id=s.id returning * into s;
 return s;
end; $$;
