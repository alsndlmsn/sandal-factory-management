-- remaining_amount عمود GENERATED ALWAYS ولا يقبل التحديث المباشر.
...
begin
  ...
  update public.sales set subtotal=sub,discount_amount=disc,vat_amount=vat,total=v_total,paid_amount=paid,payment_method=case when paid=0 then 'unpaid' when paid<v_total then 'partial' else coalesce((select sp.payment_method from public.sale_payments sp where sp.sale_id=s.id and sp.amount>0 order by sp.id limit 1),'cash') end where id=s.id returning * into s;
  return s;
end; $$;
