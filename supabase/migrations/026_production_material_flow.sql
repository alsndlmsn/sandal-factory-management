-- ربط الإنتاج بمخزن خام ومخزن منتج نهائي مع الحفاظ على البيانات الحالية.
alter table public.stock_movement_lines alter column product_id drop not null;
alter table public.stock_movement_lines drop constraint if exists stock_movement_lines_item_check;
alter table public.stock_movement_lines add constraint stock_movement_lines_item_check check (product_id is not null or raw_material_id is not null);
alter table public.production_orders add column if not exists source_warehouse_id uuid references public.warehouses(id);
alter table public.production_orders add column if not exists raw_material_id uuid references public.raw_materials(id);
alter table public.production_orders add column if not exists raw_quantity numeric(14,3) not null default 0 check(raw_quantity>=0);
...
revoke all on function public.create_production_order_v2(uuid,numeric,text,uuid,uuid,numeric,uuid,text) from public;
