create table if not exists public.return_refunds(
 ...
 sale_payment_id uuid not null references public.sale_payments(id),
 amount numeric(14,2) not null check(amount>0),
 created_at timestamptz not null default now(),
 unique(return_id,sale_payment_id)
);
...
declare r public.returns; item record; sale public.sales; si public.sale_items; mov public.stock_movements; paid_left numeric; refund_left numeric; refund_part numeric; pay record; returned_before numeric;
begin
 ...
 for item in select ri.*,si.quantity as sold_quantity from public.return_items ri join public.sale_items si on si.id=ri.sale_item_id where ri.return_id=r.id loop
   select coalesce(sum(ri2.quantity),0) into returned_before from public.return_items ri2 join public.returns rr on rr.id=ri2.return_id where ri2.sale_item_id=item.sale_item_id and rr.status='approved' and rr.id<>r.id;
 ...
 if exists(select 1 from public.return_items where return_id=r.id and good_quantity>0) then
  ...
  insert into public.stock_movement_lines(movement_id,product_id,quantity,unit_name,cost_price,notes) select mov.id,ri.product_id,ri.good_quantity,p.unit_name,p.cost_price,'مرتجع صالح '||r.return_number from public.return_items ri join public.products p on p.id=ri.product_id where ri.return_id=r.id and ri.good_quantity>0;
 end if;
 insert into public.damaged_goods(product_id,quantity,unit_name,source_type,source_id,reason,status,notes,created_by) select ri.product_id,ri.damaged_quantity,p.unit_name,'return',r.id,'تالف من مرتجع','pending','مرتجع '||r.return_number,auth.uid() from public.return_items ri join public.products p on p.id=ri.product_id where ri.return_id=r.id and ri.damaged_quantity>0;
 ...
 for pay in select sp.* from public.sale_payments sp where sp.sale_id=sale.id and sp.amount>0 order by sp.paid_at,sp.id for update loop
   ...
   if refund_part>0 then
     ...
     if pay.cash_register_id is not null then insert into public.cash_transactions(cash_register_id,direction,transaction_type,amount,related_type,related_id,created_by,notes) values(pay.cash_register_id,'out','refund',refund_part,'return',r.id,auth.uid(),'رد قيمة '||r.return_number); elsif pay.bank_id is not null then insert into public.bank_transactions(bank_id,direction,transaction_type,amount,related_type,related_id,created_by,notes) values(pay.bank_id,'out','refund',refund_part,'return',r.id,auth.uid(),'رد قيمة '||r.return_number); end if;
     refund_left:=refund_left-refund_part;
   end if;
 end loop;
 update public.sales set paid_amount=greatest(coalesce(paid_amount,0)-greatest(coalesce(r.total,0)-refund_left,0),0),remaining_amount=greatest(total-greatest(coalesce(paid_amount,0)-greatest(coalesce(r.total,0)-refund_left,0),0),0) where id=sale.id;
...
grant select,insert on public.return_refunds to authenticated;
